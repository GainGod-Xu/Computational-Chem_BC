...
if int(readxyz): 
   ...
   oldchk_name=ifs_name[:-4]+'.chk'
   shutil.copy(oldchk_name,chk_name)
...
if jobtype == 'TS':
   if int(readxyz):
      ofs.write('#p bp86/gen opt=(calcfc,ts,noeigentest) \n')
   else:
      ofs.write('#p bp86/gen opt=(readfc,ts,noeigentest) geom=check guess=read \n')

elif jobtype == 'opt':
     if int(readxyz):
        ofs.write('#p bp86/gen opt \n')
     else:
        ofs.write('#p bp86/gen opt=readfc geom=check guess=read \n')

elif jobtype == 'freq':
     if int(readxyz):
        ofs.write('#p bp86/gen freq \n')
     else:
        ofs.write('#p bp86/gen freq geom=check guess=read\n')

elif jobtype == 'scan':
     if int(readxyz):
        ofs.write('#p bp86/gen fopt=(modredundant,maxcycle=15) \n')
     else:
        ofs.write('#p bp86/gen fopt=(modredundant,maxcycle=15) geom=check guess=read \n')

elif jobtype == 'irc':
     if int(readxyz):
        ofs.write('#p bp86/gen irc=(calcfc,MaxPoints=20,stepsize=10) nosym scf(converg=5)  \n')
     else:
        ofs.write('#p bp86/gen irc=(readCartesianfc,lqa,MaxPoints=40,StepSize=20) nosym scf(converg=5)  geom=check guess=read \n')
else:
   pass
